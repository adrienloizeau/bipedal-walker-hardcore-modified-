...
class BipedalWalker(gym.Env, EzPickle):
    ...
    def __init__(self, render_mode: Optional[str] = None, hardcore: bool = False):
        ...
        self.finish_x= TERRAIN_STEP * 3 * SCALE + 500
        print(f"self.finish_x: {self.finish_x}")
    ...
    def _generate_terrain(self, hardcore):
        ...
        GRASS,TRIANGLE, ROCKS, _STATES_  = range(4)
        STUMP, STAIRS, PIT = [10,11,12]
       ...
       # Add rocks
        if hardcore:
            ...
            rock_shape = polygonShape(vertices=[(0, 0), (rock_width, 0), (rock_width, rock_height), (0, rock_height)])
            rock_fixture = fixtureDef(shape=rock_shape, density=rock_density, friction=rock_friction, restitution=rock_restitution)

            for i in range(20):
                rock_x = self.current_x + self.np_random.uniform(10, 20)
                rock_y = self.current_y + self.np_random.uniform(5, 10)
                rock_body = self.world.CreateDynamicBody(position=(rock_x, rock_y))
                rock_body.CreateFixture(rock_fixture)
                rock_body.color1,rock_body.color2 = (255,215,0),(255,215,0)
                self.terrain.append(rock_body)
        ...
        for i in range(TERRAIN_LENGTH):
            ...
            # # ---------------------------- WATER ---------------------------- #
            # elif state == WATER and oneshot:
            #     paddle_width = self.np_random.uniform(1, 10)
            #     paddle_height = self.np_random.uniform(0.2, 5)
            #     paddle_x = x - paddle_width/2
            #     paddle_y = y - TERRAIN_STEP/2 - paddle_height/2
            #     paddle_poly = [
            #         (paddle_x, paddle_y),
            ...
            #     ]
            #     self.fd_polygon.shape.vertices = paddle_poly
            #     t = self.world.CreateStaticBody(fixtures=self.fd_polygon)
            #     t.color1, t.color2 = (100, 100, 255), (100, 100, 255)
            #     self.terrain.append(t)

            # ---------------------------- ROCKS ---------------------------- #
            elif state == ROCKS and oneshot:
                poly = [(x, y), (x + TERRAIN_STEP, y), (x + TERRAIN_STEP, y + TERRAIN_STEP), (x, y + TERRAIN_STEP)]
                self.fd_polygon.shape.vertices = poly
                t = self.world.CreateStaticBody(fixtures=self.fd_polygon)
                t.color1, t.color2 = (153, 153, 153), (153, 153, 153)
                self.terrain.append(t)
            ...
            # ---------------------------- TRIANGLE ---------------------------- #
            elif state == TRIANGLE and oneshot:
                poly = [
                    (x - triangle_width * TERRAIN_STEP / 2, y),
                    (x, y+ triangle_height * TERRAIN_STEP),
                    (x + triangle_width * TERRAIN_STEP / 2, y),
                ]
                self.fd_polygon.shape.vertices = poly
                t = self.world.CreateStaticBody(fixtures=self.fd_polygon)
                t.color1, t.color2 = (102, 153, 76),(102, 153, 76)
                self.terrain.append(t)
    ...
    def reset(
        ...
        *,
        ...
        #MODIFIED init_y = TERRAIN_HEIGHT + 2 * LEG_H
        init_y = 10
        ...
        for i in [-1, +1]:
            ...
            arm = self.world.CreateDynamicBody(
                position=(init_x, init_y - ARM_H / 2),
                angle=(i * 0.05),
                fixtures=ARM_FD,
            )
            arm.color1 = (100 - i * 25, 76 - i * 25, 200 - i * 25)
            arm.color2 = (102 - i * 25, 51 - i * 25, 76 - i * 25)
            
            rjd_arm = revoluteJointDef(
                bodyA=self.hull,
                bodyB= arm,
                localAnchorA=(0, ARM_DOWN ),
            ...
            self.joints.append(self.world.CreateJoint(rjd_arm))

        self.drawlist = self.terrain + self.legs + [self.hull] + self.arms
    ...
    def step(self, action: np.ndarray):
        ...
        state = [
            ...
            1.0 if self.legs[1].ground_contact else 0.0,
            self.joints[2].angle,
            self.joints[2].speed / SPEED_SHOLDER,
            self.joints[3].angle + 1.0,
            self.joints[3].speed / SPEED_HIP,
            self.joints[4].angle + 1.0,
            self.joints[4].speed / SPEED_KNEE,
            1.0 if self.legs[3].ground_contact else 0.0,
            self.joints[5].angle + 1.0,
            self.joints[5].speed / SPEED_SHOLDER,
        ]
        state += [l.fraction for l in self.lidar]
        assert len(state) == 28
        ...
        #MODIFIED: if position is further away than the second flag
        if (pos[0] > (TERRAIN_LENGTH - TERRAIN_GRASS) * TERRAIN_STEP) or (pos[0] > self.finish_x):
            terminated = True
    ...
    def render(self):
        ...
        flagy2 = flagy1 + 50
        pygame.draw.aaline(
            self.surf, color=(0, 0, 0), start_pos=(self.finish_x, flagy1), end_pos=(self.finish_x, flagy2)
        )
    
        finish_f = [
            (self.finish_x, flagy2),
            (self.finish_x, flagy2 - 10),
            (self.finish_x + 25, flagy2 - 5),
        ]
        pygame.draw.polygon(self.surf, color=(102, 153, 76), points=finish_f)
